---
name: startup-growth
version: 1.0.0
author: wanglin1111111
description: |
  Startup Growth 创业增长技能集合，包含创业咨询顾问、增长 playbook 等技能。专注于 Product Hunt 发布、开源营销、GTM 审计、渠道选择与优先级排序，帮助种子期创业公司实现可预测的线索增长。
---

# Startup Growth

> Merged from 10 skills

## Skill: startup-consultant

---
name: startup-growth
description: |
  Startup growth consulting agent specializing in Product Hunt launches, open-source marketing, and Go-to-Market audits. Get expert-level strategic reviews and actionable recommendations. By @WeiYipei — fractional growth lead for AI startups.
---

# Startup Growth Consultant — PH Launch, OSS, GTM Audits

> 🌍 **Language / 语言**: [中文](#中文版) | [English](references/en/README.md) | [日本語](references/ja/README.md) | [한국어](references/ko/README.md)

## 📦 Install

```bash
clawhub install startup-consultant
```

**安装后你将获得：**
- Product Hunt 发布审计框架和时间策略
- OSS 增长审查（仓库、README、分发分析）
- GTM 策略审查，附带优先级的 2 周增长冲刺

---

按需咨询三大最高杠杆的创业增长挑战：

- **Product Hunt 审计**：在上线前审查你的发布资产、时机和策略
- **OSS 增长审查**：分析你的仓库、README 和分发以获取 star 速度
- **GTM 策略审查**：获取关于定位、渠道和消息传递的反馈
- **冲刺规划**：优先级的 2 周增长冲刺，附带清晰的交付物
- **Fractional growth lead**：无需全职雇用的持续战略指导

## Related Gingiris Skills
- Full version: https://clawhub.ai/skill/gingiris-growth-finder
- All skills: https://clawhub.ai/user/gingiris
- Follow: [@WeiYipei on X](https://x.com/WeiYipei)

---

## Skill: startup-growth-playbook

---
name: startup-growth
description: |
  Seed-stage growth playbook focused on channel selection and prioritization. Helps founders with product-market fit decide where to invest limited resources for maximum traction.
---

## 📦 Install

```bash
clawhub install startup-growth-playbook
```

**安装后你将获得：**
- 渠道评分矩阵，用于评估和确定渠道优先级
- 资源分配框架，最大化有限资源的牵引力

---

## 许可证

MIT License
